...
## 2026-08-11 上传5MB双重限制提示与退出组织二次确认
- 员工文档上传体积预筛由1MB同步放宽到5MB；界面同时说明服务端仍有2,000片内容上限，避免用户把“5MB可选”误解为任意5MB纯文字都能入库。大文件等待提示与F36任务跟踪继续保留。
- 员工/审核员共用组织大厅在“申请退出”前新增带组织名的二次确认，明确批准后将无法访问该组织工作区；取消不禁用按钮、不发请求，确认后才调用原有`leave-request`接口。相关脚本增加版本查询串，避免浏览器继续命中旧缓存。
- `employee.js`、`org-lobby.js`、`api.js`均通过`node --check`；隔离浏览器验证确认弹窗真实出现，取消后请求未发送，确认后请求发送并显示“退出申请已提交，等待审批”。

## 2026-08-09 v3.0交付缺口④：VERSION统一为3.0.0
- 全仓核对确认根`VERSION`只被`.github/workflows/container-ci.yml`读取，用于校验三段式版本号并生成`zhitian-admin:<version>`镜像标签；浏览器代码没有版本比较、API兼容协商或按版本切换功能的逻辑。
- `VERSION`由`2.6.0`更新为`3.0.0`，与本仓库现有`v3.0`标签对应。历史CHANGELOG中的2.6.0镜像与CI运行记录继续作为当时事实保留，不回写历史。
- 验证：CI同款版本正则校验通过并读取到`3.0.0`；现有`js/`目录10个JavaScript文件全部通过`node --check`。另以临时标签`zhitian-admin:version-verify-3.0.0`真实构建并启动容器，容器内`login.html`可访问、运行用户为非root `nginx`；验证容器与临时镜像标签随后已清理。本轮未修改任何DOM、接口地址或业务交互。
...
## 2026-08-18 补 `.gitignore` 的 env 规则

- 本仓库此前 `.gitignore` 只有 `.DS_Store`/`Thumbs.db`/`*.log`，**连 `.env` 本身都不挡**——这是 2026-08-18 用 `git check-ignore` 实测六个仓库时才发现的。顶部补入 `.env`、`.env.*`、`!.env.example` 三行，与 `zhitian-deploy` 写法一致。
- 不做路径锚定，规则在任意子目录同样生效；防的正是「在仓库目录里建 `.env` 备份」这一真实场景（2026-08-16 曾在服务器上产生 4 个含真实密钥的 `.env` 备份，已移出仓库）。
- 实测：`.env`、`.env.bak-1`、`.env.local` 均被忽略；临时落盘 `.env.example` 后 `git status` 显示为 `??` 未跟踪、未被忽略，测试文件已删除。既有三条规则未改动。

## 2026-08-18 放宽 `.gitignore` 的模板否定规则为 `!.env*.example`

- 上一轮加入的 `!.env.example` 只放行恰好同名的文件，`.env.local.example`、`.env.production.example` 这类模板会命中 `.env.*` 被静默忽略——提交时无声排除、diff 与 CI 都不报异常。本仓库改为 `!.env*.example`，只动这一行（1 增 1 删）。
- 缺陷由知了hub 执行者在本机 Compose 验证时真实撞上（新建的 `.env.local.example` 差点消失），按「两个项目共同遵守的规则必须同时写进两份」四个仓库同批跟上。
- 落盘探针实测 7 项全过：`.env`、`.env.local`、`.env.bak-1`、`.env.production`、`js/.env` 归 `!!`；`.env.example` 与 `.env.local.example` 均归 `??` 可被跟踪。实测口径按手册第十一章：不看退出码、用未跟踪探针文件加 `git status --ignored` 落盘判定。 探针已全部清理，仓库只剩 `.gitignore` 一处改动。

## 2026-08-28 存档：v3.2.1 覆盖 v3.2 之后的文档与忽略规则整理

- **本条为存档条目**。标签 `v3.2.1` 打在本条目所在的提交上，因此覆盖 `v3.2..v3.2.1` 共**五条**提交：`e7bfcdc` 版本号统一为3.2.0、`b7e0a8e` 系统性文档审计与整理、`db691ba` 补 `.gitignore` 的 env 规则、`90fb107` 放宽模板否定规则为 `!.env*.example`，以及本条存档记录本身。四条业务提交自 2026-08-18 起即已推送到 `origin/master`。
- **三段式补丁号的依据**：五条提交里没有任何应用代码、接口或用户可见行为变化——两条改 `.gitignore`、一条改 `README.md`、一条改 `VERSION`。按手册 8.1 第 2 条属「只有文档或协作流程整理」，且按 8.1「什么时候才打标签」属累积若干轮后合并打一个补丁号，不逐轮打。
- **⚠️ 如实标注一处记录缺口**：`b7e0a8e docs: 系统性文档审计与整理`（只改 `README.md`，38 增 11 删）**当时没有留下 CHANGELOG 工作条目**。本次不追补成「当时记过」——那会让记录看起来比实际更完整。缺口就是缺口，标签注释里同样写明。
- **`VERSION` 本轮不动，保持 `3.2.0`**。它在 `v3.2` 标签处还是 `3.0.0`，是被本批的 `e7bfcdc` 升到 3.2.0 的，已经与 v3.2 功能基线对齐；三段式补丁号不再上抬版本源。本仓库的 `VERSION` 仅由 `container-ci.yml` 做格式校验，不像后端那样卡容器启动。
- **CI 结论（打标前实查，非转述）**：四条提交上 `CI` 与 `Admin Container CI` 两条 workflow **全部 success**，无红灯。HEAD `90fb107` 的两条分别为 runs/32321409634 与 runs/32321409701。存档前例行核对：IPv4 字面量 0 处、密钥凭据形态 0 处、服务器绝对路径 0 处、主机名 0 处、未跟踪文件 0 个、二进制改动 0 处。
